...
const useMessages = ({userId, contactId}:{userId: string; contactId: string}) => {
    const {data : oldMessages, isSuccess, isError, error} = useGetUserContactMessagesQuery({userId: userId as string, contactId: contactId as string})
    ...
    return{messages, setMessages, isError, error, receiveNewMessage}
}
